...
class BinarySearchTree : public trees::BinaryTree<T> {
 ...
 protected:
  typename BinaryTree<T>::Node *Search(typename BinaryTree<T>::Node *node, const T &data) const;
  typename BinaryTree<T>::Node *Maximum(typename BinaryTree<T>::Node *node) const;
  typename BinaryTree<T>::Node *Minimum(typename BinaryTree<T>::Node *node) const;
};

template <class T>
typename BinaryTree<T>::Node *BinarySearchTree<T>::Search(typename BinaryTree<T>::Node *node, const T &data) const {
  if (node == NULL || node->data_ == data) return node;
  return data < node->data_ ? Search(node->left_, data) : Search(node->right_, data);
}
...
T BinarySearchTree<T>::Remove(const T &data) {
  if (this->IsEmpty()) throw std::runtime_error("Tree is empty, can not remove");
...
const typename BinaryTree<T>::Node *BinarySearchTree<T>::Maximum() const {
  if (this->IsEmpty()) throw std::runtime_error("Tree is empty, can not get maximum");
  return Maximum(this->root_);
}

template <class T>
typename BinaryTree<T>::Node *BinarySearchTree<T>::Maximum(typename BinaryTree<T>::Node *node) const {
...
const typename BinaryTree<T>::Node *BinarySearchTree<T>::Minimum() const {
  if (this->IsEmpty()) throw std::runtime_error("Tree is empty, can not get minium");
  return Minimum(this->root_);
}

template <class T>
typename BinaryTree<T>::Node *BinarySearchTree<T>::Minimum(typename BinaryTree<T>::Node *node) const {
  if (this->IsEmpty()) return NULL;
