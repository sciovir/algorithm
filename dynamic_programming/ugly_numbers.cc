...
unsigned int UglyNumber(unsigned int n) {
  ...
  for (unsigned int i = 1; i < n; i++) {
    next_ugly_number = std::min(multiplication_2, std::min(multiplication_3, multiplication_5));
    ugly_numbers[i] = next_ugly_number;
    if (multiplication_2 == next_ugly_number) multiplication_2 = ugly_numbers[++i2] * 2;
    if (multiplication_3 == next_ugly_number) multiplication_3 = ugly_numbers[++i3] * 3;
    if (multiplication_5 == next_ugly_number) multiplication_5 = ugly_numbers[++i5] * 5;
  }
