...
def rat_in_maze(
    board: list[list[int]], x: int, y: int, dx: int, dy: int
) -> list[list[int]]:
...
def rat_in_maze_recursive(
    board: list[list[int]], out: list[list[int]], x: int, y: int, dx: int, dy: int
) -> bool:
    if possible_move(board, x, y):
        ...
        if rat_in_maze_recursive(board, out, x + 1, y, dx, dy) or rat_in_maze_recursive(
            board, out, x, y + 1, dx, dy
        ):
            return True
        out[x][y] = 0
...
class TestRatInMaze(unittest.TestCase):
    def test_rat_run_from_top_left_to_bottom_right(self):
        ...
        result: list[list[int]] = rat_in_maze(
            board, 0, 0, len(board) - 1, len(board) - 1
        )
