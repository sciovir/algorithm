 ...
 *
 * Examples:
 *  - Giving an array [5, 2, 3, 1, 4], assuming that we sort the array by ascending order.
 *  - Starting with the first index, we compare the element at current index with the rest.
 *  At the end of first loop, we get the smallest element at current index.
 *     0  1  2  3  4 -> array's indexes
 *    [5, 2, 3, 1, 4] we compare the first 2 elements then swap 5 and 2 cause 5 > 2
 *     i  j
 *    [2, 5, 3, 1, 4] since these elements are in order (2 < 3), we don't swap them
 *     i     j
 *    [2, 5, 3, 1, 4] swap 2 and 1
 *     i        j
