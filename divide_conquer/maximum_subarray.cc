...
std::tuple<int, int, int> MaxCrossingSubarray(const int *array, int low, int mid, int high) {
  ...
  if (l_sum > r_sum && l_sum > (l_sum + r_sum)) return std::make_tuple(l_max, mid, l_sum);
  else if (r_sum > l_sum && r_sum > (l_sum + r_sum)) return std::make_tuple(mid + 1, r_max, r_sum);
  else return std::make_tuple(l_max, r_max, l_sum + r_sum);
}
...
std::tuple<int, int, int> MaximumSubarray(const int *array, int low, int high) {
  ...
  if (std::get<2>(l_tuple) >= std::get<2>(r_tuple) && std::get<2>(l_tuple) >= std::get<2>(c_tuple))
    return l_tuple;
...
int main() {
  ...
  std::tuple<int, int, int> ret =
      algorithms::divide_conquer::MaximumSubarray(arr, 0, sizeof(arr) / sizeof(*arr));
